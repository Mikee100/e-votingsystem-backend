...
async function getCurrentSchema() {
  const query = "SELECT current_schema FROM evoting_system.settings LIMIT 1;";
  const [rows] = await db.execute(query);
  return rows.length > 0 ? rows[0].current_schema : null;
}
...
app.get("/api/dashboard-stats", async (req, res) => {
  ...
  try {
    ...
    const [candidatesResult] = await db.query(
      `SELECT COUNT(*) AS count FROM ${schemaName}.candidates`
    );
...
app.get("/api/presidential-votes-chart", async (req, res) => {
  ...
  try {
    const [voteCounts] = await db.query(`
      ...
      JOIN  ${schemaName}.parties p ON v.party_id = p.id
...
app.get("/schools", async (req, res) => {
  try {
    ...
    const [schools] = await db.query(
      "SELECT idschools, schoolname FROM schools"
    );
...
app.get("/api/congresspersons", async (req, res) => {
  try {
    ...
    const [congresspersons] = await db.query(
      "SELECT * FROM congresspersonroles"
    );
...
app.get("/departments/:schoolId", async (req, res) => {
  ...
  try {
    ...
    const [departments] = await db.query(
      "SELECT iddepartments, department_name FROM departments WHERE school_id = ?",
      [schoolId]
    );
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const verificationUrl = `http://localhost:5173/verify?token=${encodeURIComponent(
      token
    )}`;
...
app.get("/api/getCongressPeopleByCampus", async (req, res) => {
  ...
  try {
    ...
    const voteQuery = `
      ...
      WHERE user_hash = ? AND candidate_type = 'Campus Congressperson';
    `;
    const [voteData] = await db.execute(voteQuery, [email]);

    const votedCandidateId =
      voteData.length > 0 ? voteData[0].candidate_id : null;
    ...
    if (congressPeople.length > 0) {
      return res.json({
        ...
        campusId,
        votedCandidateId,
      }); // Include both campus name and ID
    } else {
      console.log("No candidates found for the specified campus and role");
      return res
        .status(404)
        .send("No candidates found for the specified campus and role");
    }
  } catch (err) {
...
app.get("/api/getCongressPeopleByStudentType", async (req, res) => {
  ...
  try {
    ...
    // Step 2: Fetch congressperson_type ID
    const roleQuery = `
      SELECT id
    ...
    if (roleData.length === 0) {
      return res.status(404).send("No congressperson role found");
    }

    const { id: congresspersonTypeId } = roleData[0];

    // Step 3: Fetch congresspeople
    const congressPeopleQuery = `
      SELECT *
      FROM ${schemaName}.candidates
    ...
    const [congressPeople] = await db.execute(congressPeopleQuery, [
      congresspersonTypeId,
    ]);

    // Step 4: Fetch the user's vote using the hashed email
    const voteQuery = `
      SELECT candidate_id
      FROM ${schemaName}.full_votes
      WHERE user_hash = ? AND candidate_type = ?;
    `;
    const [voteData] = await db.execute(voteQuery, [hashedEmail, student_type]);
  ...
  try {
    ...
    if (campus !== 1) {
      ...
      return res
        .status(403)
        .send(
          "Access denied. Only main campus users can view these candidates."
        );
    }
    ...
    const voteQuery = `
      SELECT candidate_id
      FROM ${schemaName}.full_votes
      WHERE user_hash = ? AND candidate_type = 'Non-Resident Congressperson';
    `;
    const [voteData] = await db.execute(voteQuery, [email]);
    ...
    if (Residentcandidates.length > 0) {
      ...
      console.log(
        "No candidates found for residency status:",
        residency_status
      );
      return res
        .status(404)
        .send(
          "No candidates found for the specified residency status in the main campus"
        );
    }
  } catch (err) {
    console.error("Error fetching data:", err);
...
app.get("/api/getCongressPeopleByDisability", async (req, res) => {
  ...
  try {
    ...
    if (disabled !== 1) {
      ...
      return res
        .status(403)
        .send("Access denied. Only Disabled users can view these candidates.");
    }
    ...
    const voteQuery = `
      SELECT candidate_id
      FROM ${schemaName}.full_votes
      WHERE user_hash = ? AND candidate_type = 'Disabled Congressperson';
    `;
    const [voteData] = await db.execute(voteQuery, [email]);
...
app.get("/api/getCongressPeopleByInternational", async (req, res) => {
  ...
  try {
    ...
    if (internationalstudent !== 1) {
      return res
        .status(403)
        .send("User is not eligible to see international candidates");
    }
    ...
    const voteQuery = `
      SELECT candidate_id
      FROM ${schemaName}.full_votes
      WHERE user_hash = ? AND candidate_type = 'International Congressperson';
    `;
    const [voteData] = await db.execute(voteQuery, [email]);
...
const getCandidatesBySchool = async (
  schoolId,
  userEmail,
 
) => {
  const globalSchema = 'evoting_system';
  try {
    let schemaName;
    try {
       schemaName = await getCurrentSchema();
...
app.get("/candidates", async (req, res) => {
  ...
  if (!schoolId || !email) {
    return res
      .status(400)
      .json({ error: "Missing schoolId or email parameter" });
  }

  try {
    const { candidatesBySchool, candidatesByHostel, population } =
      await getCandidatesBySchool(schoolId, email);
...
app.get("/api/candidates/school/:schoolId", async (req, res) => {
  ...
  try {
    ...
    if (candidates.length === 0) {
      return res
        .status(404)
        .json({ message: "No candidates available for this school." });
    }
...
app.post("/vote", async (req, res) => {
  ...
  try {
    ...
    const existingVote = await db.query(
      "SELECT * FROM votes WHERE voter_email = ? AND candidate_id = ?",
      [email, leaderId]
    );
    if (existingVote.length > 0) {
      return res
    ...
    // Insert the vote
    await db.query(
      "INSERT INTO votes (candidate_id, voter_email, role) VALUES (?, ?, ?)",
      [leaderId, email, role]
    );
...
app.get("/votes/tally", async (req, res) => {
  try {
    ...
    const hostelReps = await db.query(
      'SELECT candidate_id, COUNT(*) AS votes FROM votes WHERE role = "Hostel Rep" GROUP BY candidate_id'
    );
    tally.hostelReps = hostelReps;

    // Tally votes for Delegates
    const delegates = await db.query(
      'SELECT candidate_id, COUNT(*) AS votes FROM votes WHERE role = "Delegate" GROUP BY candidate_id'
    );
    tally.delegates = delegates;

    // Tally votes for Congress Persons
    const congressPersons = await db.query(
      'SELECT candidate_id, COUNT(*) AS votes FROM votes WHERE role = "Congress Person" GROUP BY candidate_id'
    );
...
app.post("/vote/congressperson", async (req, res) => {
  ...
  try {
    ...
    // Check if the user has already voted in this category (anonymous, no email check)
    const [existingVote] = await db.query(
      `SELECT leader_id FROM ??.congressperson_votes WHERE leader_id = ? AND school_id = ? AND email = ? LIMIT 1`,
    ...
    // Insert vote anonymously (with hashed email)
    await db.query(
...
app.post("/vote/delegate", async (req, res) => {
  ...
  try {
    ...
    const hashedEmail = HashEmail(email); // Hash the email before using it
...
app.post("/api/elections", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Election created successfully", id: result.insertId });
  } catch (error) {
...
app.put("/api/elections/:id/status", async (req, res) => {
  ...
  try {
    await db.query("UPDATE elections SET status = ? WHERE id = ?", [
      status,
      id,
    ]);
...
app.get("/api/elections/:id/status", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      "SELECT end_date FROM elections WHERE id = ?",
      [id]
    );
    if (rows.length === 0) {
  ...
  try {
    ...
    if (existingVote.length > 0) {
      return res
        ...
        .json({
          success: false,
          message: "You have already voted for this role!",
        });
    }
...
app.get("/voting-status", async (req, res) => {
  ...
  try {
    const votes = await db.query("SELECT role FROM votes WHERE email = ?", [
      email,
    ]);
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching voting status." });
  }
});
...
app.post("/saveVote", async (req, res) => {
  ...
  try {
    ...
    if (existingVote.length > 0) {
      ...
      return res
        .status(400)
        .json({ message: "You have already voted for this category." });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Vote saved successfully" });
  } catch (error) {
...
app.post("/leaders", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Leader created successfully" });
  } catch (error) {
...
app.put("/leaders/:id", async (req, res) => {
  ...
  try {
    const query =
      "UPDATE leaders SET party_id = ?, position = ?, name = ? WHERE id = ?";
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ success: true, message: "President registered successfully" });
    } catch (error) {
      console.error("Error registering president:", error);
      res
        .status(500)
        .json({ success: false, message: "Failed to register president" });
    }
  }
);
...
`,
    ...
    const totalVotes = performanceData.reduce(
      (acc, data) => acc + data.total_votes,
      0
    );
    const maxVotes = Math.max(
      ...performanceData.map((data) => data.total_votes)
    );
...
app.post("/api/vote-president", async (req, res) => {
  ...
  const schemaName = `evoting_${year}`; // Dynamic schema name
  const secretKey = process.env.SECRET_KEY || "wawer"; // Replace with a secure key from your environment
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while processing your vote." });
  }
});

app.post("/api/get-voted-candidate", async (req, res) => {
  const { email } = req.body;
  const year = req.query.year || new Date().getFullYear();
  const schemaName = `evoting_${year}`;
  const secretKey = process.env.SECRET_KEY || "wawer";
  ...
  try {
    ...
    const [result] = await db.query(
      `SELECT party_id FROM ${schemaName}.presidential_votes WHERE anonymous_id = ?`,
      [anonymousId]
    );
    ...
    res.status(200).json({ selectedCandidate: result[0].party_id });
  } catch (error) {
    console.error("Error fetching voted candidate:", error);
    res
      .status(500)
      .json({ message: "An error occurred while fetching the vote." });
  }
});
...
app.post("/api/verify-vote", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while verifying your vote." });
  }
});
  ...
  try {
    ...
    if (voterDetails.length === 0) {
      return res
        .status(404)
        .json({
          message: "No matching voter found for the given anonymous number.",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while looking up the voter." });
  }
});
...
app.post("/api/admin/lookup-voter", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Vote not found or anonymous ID is invalid." });
    }
  ...
  try {
    ...
    if (standings.length === 0) {
      return res
        .status(404)
        .json({
          message: `No presidential standings found for the year ${year}.`,
        });
    }
...
app.get("/api/announcements", async (req, res) => {
  try {
    const [announcements] = await db.query(
      "SELECT * FROM announcements ORDER BY scheduled_date DESC"
    );
...
app.post("/api/announcements", upload.single("image"), async (req, res) => {
  ...
  try {
    const formattedDate = new Date(scheduled_date)
      ...
      .replace("T", " ");
    await db.query(
      "INSERT INTO announcements (title, content, scheduled_date, image_path) VALUES (?, ?, ?, ?)",
      [title, content, formattedDate, imagePath]
    );
...
app.put("/api/announcements/:id", upload.single("image"), async (req, res) => {
  ...
  try {
    const formattedDate = new Date(scheduled_date)
      ...
      .replace("T", " ");
    await db.query(
      "UPDATE announcements SET title = ?, content = ?, scheduled_date = ?, image_path = ? WHERE id = ?",
      [title, content, formattedDate, imagePath, id]
    );
...
app.put("/campuses/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Campus updated successfully", representatives });
  } catch (err) {
...
app.post("/api/generate-token", async (req, res) => {
  ...
  try {
    ...
    console.error("Error fetching schema:", err);
    return res.status(500).send("Error fetching schema");
  } // Dynamic schema name for the current year
  ...
  if (!voterId || !candidateType) {
    return res
      .status(400)
      .json({ error: "Voter ID and Candidate Type are required" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to generate token. Please try again later." });
  }
});
...
app.get("/api/candidates/all", async (req, res) => {
  ...
  try {
    ...
    const [candidates] = await db.query(
      `SELECT * FROM ${schemaName}.candidates`
    );
...
app.get(
  "/api/admin/congressvote-stats/congressperson/:id",
  async (req, res) => {
    let schemaName;
    try {
       schemaName = await getCurrentSchema();
    ...
    const roleQuery =
      "SELECT congressname FROM congresspersonroles WHERE id = ?";
    const voteQuery = `
  SELECT 
    fv.candidate_id, 
...
app.post("/api/campus-vote", async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 1) {
      ...
      return res
        .status(200)
        .json({ message: "Your vote has been updated successfully!" });
    }
  } catch (err) {
...
app.get("/api/stats/campus-vote-stats/campus/:campusId", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: `No vote stats found for campus: ${campusId}` });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching campus stats." });
  }
});
...
app.get("/api/getSchemaYear", async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).send("Schema year not set");
    }
  
    res.json({ schemaYear: result[0].current_schema });
  } catch (err) {
    console.error("Error fetching schema year:", err);
    res.status(500).send("Error fetching schema year");
  }
});
...
app.post("/api/updateSchemaYear", async (req, res) => {
  ...
  if (!schemaYear || isNaN(schemaYear)) {
  ...
  try {
    ...
    res.json({ message: "Schema year updated successfully", schemaYear });
  } catch (err) {
    console.error("Error updating schema year:", err);
    res.status(500).send("Error updating schema year");
  }
});


app.get("/api/getCongressResults", async (req, res) => {
  let schemaName;
  try {
    schemaName = await getCurrentSchema();
    if (!schemaName) {
  ...
  try {
    // Fetch all schools
    const schoolQuery = `
      SELECT DISTINCT s.idschools AS school_id, s.schoolname AS school_name
      FROM schools s
      JOIN ${schemaName}.candidates c ON s.idschools = c.school_id
    `;
    const [schools] = await db.execute(schoolQuery);

    // Fetch all candidates with votes
    const candidatesQuery = `
      SELECT c.id AS candidate_id, c.photo_path, c.name AS candidate_name, c.school_id, 
    ...
    const results = schools.map((school) => ({
      ...
      candidates: candidates
        .filter((c) => c.school_id === school.school_id) // Group candidates by school
        .map((c) => ({
